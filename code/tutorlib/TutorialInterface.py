...
class TutorialInterface:
    ...
    def _parse_text(self, text):
        ...
        if not self.data:
            print >> sys.stderr,'Tutorial file has incorrect format'
    ...
    def get_tut_zipfile(self):
        ...
        if '_send_data Exception' in result:
            print >> sys.stderr, "You don't appear to be connected."
            return None
        try:
    ...
    def print_exception(self, e):
        ...
        print >> sys.stderr, '%s: %s\nTry Googling  the error type for help.' %  (type(e).__name__, str(e))
        self.set_fail()
    ...
    def print_exception_info(self, e):
        ...
        if traceinfo[0] == USER_CODE_FILE:
            print >> sys.stderr, "line %d, in %s" % (traceinfo[1], traceinfo[2])
            print >> sys.stderr, self.user_text.split('\n')[line-1].strip()
    ...
    def run_tests(self, text):
        ...
        try:
            ...
            alarm.stop_interrupt()
            print >> sys.stderr, 'Timeout - possible infinite loop'
    ...
    def _runtests(self):
        ...
        if self.get_text():
            ...
            try:
                ...
                try:
                    exec self.data.get('GlobalCode', '') in global_env
                except Exception, e:
                    raise TestError(e)
                ...
                for test in tests:
                    for _ in xrange(test['repeats']):
                        ...
                        try:
                            exec test.get('start', '') in test_globals
                        except Exception, e:
                            raise TestError(e)
                        ...
                        try:
                            exec test.get('init', '') in test_globals,locs
                        except Exception, e:
                        ...
                        try:
                            ...
                            # user code
                            exec test.get('code', '') in test_globals
                        except NameError,e:
                            raise(NameError(e))
                        except Exception, e:
                            raise TestError(str(e))
...
class TutParser:
    ...
    def _parse_it(self, parse_code, enc):
        while True:
            ...
            if tok_type == 'header':
                ...
                if header == 'TestCode':
                    ...
                    if parse_code:
                        ...
                        if tests:
                            ...
                            print >> sys.stderr, "No tests"
                            return False
                    else:
                        self.data['TestCode'] = code
                    break
                if not self._next_token():
                    print >> sys.stderr, "Nothing after header"
                ...
                print >> sys.stderr, "Expecting header"
                raise ParseError, pos2linenum(self.string, self.pos)
            if not self._next_token():
                break
        return True
    ...
    def parse(self, string, parse_code, enc = True):
        ...
        self.pos = 0
        if not self._next_token():
            print >> sys.stderr, "Empty string"
            return None
        while self.curr_token[0] != 'header':
            if not self._next_token():
                print >> sys.stderr, "No headers found"
                return None
        ...
        try:
            ...
            print >> sys.stderr, "Parse error in text at line", e.pos
            return None
        except Exception, e:
            print str(e)
        ...
        try:
            ...
            while True:
                ...
                if first_token[0] != 'header':
                    print >> sys.stderr, "Must start with header"
                    return (None,None,None,None,None)
                if first_token[1] in ['preload','global']:
                    next_token = tokens.pop(0)
                    if next_token[0] != 'body':
                        print >> sys.stderr, "Header must be followed by body"
                        return (None,None,None,None,None)
                    ...
                    if len(args) != 2 or 'id' != args[0].strip():
                        print >> sys.stderr, "Invalid syntax '%s'" %\
                            first_token[1]
                        return (None,None,None,None,None)
                    else:
                    ...
                    print >> sys.stderr, "Must be 'test' header"
                    return (None,None,None,None,None)
                else:
                    break
            ...
            while tokens:
                ...
                if next_token[0] != 'header':
                    print >> sys.stderr, "Must be header"
                    return (None,None,None,None,None)
                header = next_token[1]
                ...
                if 'test' in header:
                    ...
                    if test_dict:
                        ...
                        print >> sys.stderr, "test must contain data"
                        return (None,None,None,None,None)

                at_test = False
                if header not in ['start', 'init', 'code']:
                    print >> sys.stderr, "unknown header: "+header
                    return (None,None,None,None,None)
                next_token = tokens.pop(0)
                if next_token[0] != 'body':
                    print >> sys.stderr, "Header must be followed by body"
            ...
            print >> sys.stderr, "Parse error in code"
            return (None,None,None,None,None)
...
def extract_test_config(text):
    ...
    for argpair in argpairs:
        if len(argpair) != 2:
            print >> sys.stderr, "Invalid syntax for test arguments."
            return None
        arg1 = argpair[0].strip()
        arg2 = argpair[1].strip()
        if arg1 != 'repeats':
            print >> sys.stderr, "Invalid syntax for test arguments '%s'." % arg1
            return None
        try:
            argnum = int(arg2)
        except:
            print >> sys.stderr, "Invalid syntax for test arguments - int required - got '%s'." % arg2
            return None
...
class Trans:
    def __init__(self, x=0):
        ...
        if x % 5381 == 1879 and x / 5381 == 14:
            ...
            self.n3 = n0**n2
            self.n4 = self.n3*4-1
            self.n5 = range(self.n3, self.n4-1)
    ...
    def _trans1(self):
        ...
        lst3 = lst1[:n2]
        lst4 = lst1[n2:]
        return dict(zip(lst3,lst4)+zip(lst4,lst3))
